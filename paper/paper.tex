...
the results match. The reason is technical and is described in
Section~\ref{sec:coq-techniques} alongside the techniques we implemented in Coq
...
\asi{We should have a sentence like ``Proving the other direction requires doing
  an inversion of the full JSCert semantics. Unfortunately, the techniques we
  have used (see Section $\ldots$) do not scale to semantics of that size.'' }
